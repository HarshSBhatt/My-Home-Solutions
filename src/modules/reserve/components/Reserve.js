...
export default function Reserve() {
    ...
    function callProperties(event) {
        ...
        if(validateDate(event)){
            api.get('/getProperty/get-property-details').then(response => {
                const filteredData = response['data'].filter((property) => new Date(property.availabilityStartDate).getTime() <=  new Date(startDate).getTime())
    ...
    function addToCart(id,rent){
        ...
        api.post('cart/add', postData).then(response => {
                ...
                {properties().map((data, id) => (
                    <Grid container spacing={2} columns={16} margin={10} key={id}>
                        ...
                        <Grid item xs={12} md={6} marginLeft={20} marginTop={10}>
                            ...
                            <p> Amenities Include:
                                {data.amenities.map((amenity) => <Button key={amenity} variant="text" >
                                    {amenity}</Button> )}
                            </p>
                            <Button variant="contained" onClick={()=> addToCart(data._id, data.rent)}>Add To Cart</Button>

                            {success && <Alert severity="success">Property has been added to cart</Alert>}
                        </Grid>
                    </Grid>
                ))}
            </>)
    ...
    return (
        ...
        <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }} marginTop={3}>
            ...
            <Grid item xs={12} md={3}>
                <Button variant="contained" onClick={callProperties} >Search</Button>
            </Grid>
            {/*<Grid item xs={12} md={3}>*/}
            {/*    <Button variant="contained" onClick={callBooking} >Test</Button>*/}
            {/*</Grid>*/}
            {error ? <div style={{marginLeft : "5px"}} ><Alert severity="error">{error} </Alert> </div> : displayProperties()}
        </Grid>
    );
}